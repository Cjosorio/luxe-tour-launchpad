...
export const Coverage = () => {
  const { t } = useI18n();
  return (
    <section id="coverage" className="relative py-28 md:py-40 bg-background">
      <div className="container">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
              ...
              <ul className="mt-10 space-y-4">
                {t.coverage.zones.map((z, i) => (
                  <li key={z} className="flex items-center gap-4 group">
                    <span className="text-[10px] tracking-[0.25em] font-semibold text-primary w-6">
                      0{i + 1}
                    </span>
                    <span className="h-px flex-1 bg-foreground/15 group-hover:bg-primary transition-colors" />
                    <span className="text-foreground font-medium text-base">{z}</span>
          ...
          <div className="lg:col-span-7">
            <Reveal delay={0.15}>
              <div className="relative aspect-[4/3] overflow-hidden rounded-sm shadow-elegant border border-foreground/10">
                <iframe
                  title="Cobertura"
                ...
                <div className="pointer-events-none absolute top-4 left-4 bg-primary text-primary-foreground px-3 py-1 text-[10px] tracking-[0.3em] uppercase font-semibold">
                  Operativos
                </div>
              </div>
            </Reveal>
